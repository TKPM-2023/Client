...
type Schema = Pick<UserSchema, 'first_name' | 'last_name' | 'phone'>
const schema = userSchema.pick(['first_name', 'last_name', 'phone'])

function EditModal({ user, isOpen, setIsOpen, handleRefetchData }: Props) {
  const updateUserMutation = useMutation({
    mutationFn: (body: Schema) => userApi.updateUser(user.id, body)
  })
  ...
  if (!user) return null
  return (
    <Modal headingTitle='Sửa thông tin người dùng' isOpen={isOpen} setIsOpen={setIsOpen}>
      <form onSubmit={onSubmit}>
        <div className='py-4'>
          <div className='grid grid-cols-12 gap-3'>
            <div className='col-span-12'>
              <div className='text-sm font-medium'>ID</div>
              <Input className='mt-2' disabled value={user?.id} />
            </div>
            ...
            <div className='col-span-3'>
              ...
              <Input className='mt-2' name='last_name' disabled value={renderRole(user.role)} />
            </div>

            <div className='col-span-3'>
              <div className='text-sm font-medium'>Trạng thái</div>
              <Input className='mt-2' disabled value={user?.status} />
            </div>
          </div>
        </div>
