  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (isAxiosBadRequestError<ErrorResponse>(error)) {
        ...
        let errorName: FieldPath<CategorySchema> | null = null

        if (formError?.error_key === 'ErrCategoryNameLengthIsInvalid') errorName = 'name'
        else if (formError?.error_key === 'ErrCategoryDescriptionLengthIsInvalid') errorName = 'description'

        if (errorName && formError) {
          setError(errorName, {
    ...
    <Modal headingTitle='Sửa thông tin sản phẩm' isOpen={isOpen} setIsOpen={setIsOpen}>
      <form onSubmit={onSubmit}>
        ...
        <div className='flex items-center justify-end gap-1 px-4 py-3'>
          ...
          >
            <span className='relative rounded-md bg-white px-5 py-2.5 transition-all duration-75 ease-in group-hover:bg-opacity-0 dark:bg-gray-900'>
              {(updateCategoryMutation.isLoading || uploadImageMutation.isLoading) && (
                <svg
                  aria-hidden='true'
                  role='status'
